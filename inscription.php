	...
	if(!empty($_POST['pseudo']) AND !empty($_POST['mail']) AND !empty($_POST['mail2']) AND !empty($_POST['mdp']) AND !empty($_POST['mdp2']))
	{
		...
		if($pseudolength <= 255)
		{
			if($mail == $mail2)
			{
				...
				{
					$reqmail = $bdd->prepare("SELECT * FROM membrepeda WHERE mail = ?");
					...
					if($mailexist ==0)
					{
						if($mdp == $mdp2)
						{
							$insertmbr = $bdd->prepare("INSERT INTO membrepeda(pseudo, mail, motdepasse) VALUES(?, ?, ?)");
...
<html>
	...
	<body>
		<div align="center">
			<h1>Inscription membre équipe pédagogique</h1>
			<br /><br />

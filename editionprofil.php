...
if(isset($_SESSION['id']))
{
	...
	if(isset($_POST['newpseudo']) AND !empty($_POST['newpseudo']) AND $_POST['newpseudo'] != $user['pseudo'])
	{
		...
		$insertpseudo = $bdd->prepare("UPDATE membrepeda SET pseudo = ? WHERE id = ?");
	...
	if(isset($_POST['newmdp1']) AND !empty($_POST['newmdp1']) AND isset($_POST['newmdp1']) AND !empty($_POST['newmdp2']))
	{
		...
		if($mdp1 == $mdp2)
		{
			$insertmdp = $bdd->prepare("UPDATE membrepeda SET mot de passe = ? WHERE id = ?");
	...
	if(isset($_FILES['avatar']) AND !empty($_FILES['avatar']['name']))
	{
		...
		if($_FILES['avatar']['size'] <= $tailleMax)
		{
			...
			if(in_array($extensionUpload, $extensionsValides))
			{
				...
				if($resultat)
				{
					$updateavatar = $bdd->prepare('UPDATE membrepeda SET avatar = :avatar WHERE id = id:');
